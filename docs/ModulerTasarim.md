...
┌──────────────▼────────────────┐   ┌──────▼─────────────────────┐
│   SemptomAnalizApp.Service    │   │   SemptomAnalizApp.Data    │
│   (Business Logic Services)   │   │   (EF Core, Seeder, UoW)   │
└──────────────┬────────────────┘   └──────┬─────────────────────┘
               │ depends on                │ depends on
┌──────────────▼───────────────────────────▼───────────┐
...
İş mantığı katmanı. Core'a bağımlıdır; veri erişimi için Core'daki `IAnalizDbContext`
sözleşmesini kullanır. Somut `AppDbContext` Web katmanında DI ile bağlanır.

| Dosya | İçerik |
|-------|--------|
| `Interfaces/IAnalizService.cs` | Analiz motoru sözleşmesi + `SemptomGirdisi` record |
| `Interfaces/I*Service.cs` | BMI, imza, Bayes, aciliyet, tekrar, metin ve öneri servis sözleşmeleri |
| `Services/AnalizMotoru.cs` | Analiz akışını yöneten orkestrasyon servisi |
| `Services/BayesianAnalizService.cs` | Naive Bayes log-likelihood hesabı |
| `Services/BmiService.cs` | BMI ve BMI kategorisi hesabı |
| `Services/AciliyetService.cs` | Aciliyet skoru ve seviye hesabı |
| `Services/TekrarAnalizService.cs` | 30 günlük tekrar imzası kontrolü |
| `Services/AnalizMetinService.cs` | Açıklama ve genel yorum üretimi |
| `Services/GunlukOneriService.cs` | Günlük öneri ve uyarı üretimi |
...
- Algoritma parçaları küçük servislerde ayrıştırılmıştır → **Encapsulation**
...
│   │   └── Services/                # AnalizMotoru ve küçük analiz servisleri
│   │
